...
#wordcloud2(demoFreq_f,shape = 'circle')
...
df_lda<-LDA(df_tm,k=30,control = list(seed=0))
df_lda
```
...
df_new2<-data_frame(category=hm_data$predicted_category,marital=hm_data$marital,gender=hm_data$gender,text=as.character(hm_data$text),category_marital=paste(as.character(hm_data$predicted_category),as.character(hm_data$marital),as.character(hm_data$gender),sep = "."))
df_tm2<-df_new2 %>%
 unnest_tokens(word,text)
df_tm2
...
```{r echo=FALSE, results='hide',message=FALSE}
df_tm2<-df_tm2 %>% 
  count(category_marital,word,sort = TRUE)

df_tm2<-df_tm2%>%
   cast_dtm(category_marital,word,n)
df_tm2


df_lda2<-LDA(df_tm2,k=16,control = list(seed=0))
df_lda2
```
...
df_lda_compare <- n_topics %>%
  map(LDA, x = df_tm2, control = list(seed = 1109))
```


```{r}
data_frame(k = n_topics,
           perplex = map_dbl(df_lda_compare, perplexity)) %>%
